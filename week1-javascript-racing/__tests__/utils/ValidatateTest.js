...
describe('Validate 클래스 테스트', () => {
  ...
  describe('검증 메서드 테스트', () => {
    ...
    test('isNotUndefined 함수는 undefined가 아닌 값을 검증한다.', () => {
      ...
      expect(validate.isNotUndefined(undefined)).toBe(
        '입력값이 undefined입니다.',
      );
    });
    ...
    test('checkMultiPlayer 함수는 플레이어가 2명 이상인지 검증한다.', () => {
      ...
      expect(validate.checkMultiPlayer(['pobi'])).toBe(
        '플레이어는 두 명 이상이어야 합니다.',
      );
    });
    ...
    test('checkNameMaxLength 함수는 자동차 이름의 길이가 5자 이하인지 검증한다.', () => {
      ...
      expect(validate.checkNameMaxLength(['pobipobi'])).toBe(
    ...
    test('checkNameUnique 함수는 자동차 이름이 고유한지 검증한다.', () => {
      ...
      expect(validate.checkNameUnique(['pobi', 'pobi'])).toBe(
        '자동차 이름은 중복될 수 없습니다.',
      );
    });
    ...
    test('isNaturalNumber 함수는 시도할 횟수가 자연수인지 검증한다.', () => {
      ...
      expect(validate.isNaturalNumber(0)).toBe(
        '시도할 횟수는 자연수여야 합니다.',
      );
    });
  });
